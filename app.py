...
def image_res():
    st.title("Image Retargeting Tool")
    st.markdown(
        """
    Draw over the parts of the image you want removed or want to keep, this tool will try to remove it/resize your image using a seam carving algorithm.
    The number of seams equate to the number of pixels you want to remove from that dimension. One vertical seam removes one pixel from the width dimension. 
    """
    )
    uploaded_file = st.file_uploader("Choose a file")
    if uploaded_file is not None:
        ...
        width, height = up_image.size
        stroke_width = st.sidebar.slider("Stroke width: ", 1, 100, 20)
        vertical_seams = st.sidebar.slider("Vertical seams: ", -width, width, 0, step=1)
        horizontal_seams = st.sidebar.slider("Horizontal seams: ", -height, height, 0,step=1)
        show_mask = st.sidebar.checkbox('Show mask')
        ...
        if canvas_result.image_data is not None:
            ...
            if show_mask:
                ...
                st.image(im)
            if st.button('Run'):
                st.write("Image with seam-carving:")
                resized_img = s_image(np.array(up_image),np.array(im),vertical_seams,horizontal_seams,mode=type) #TODO Put button here
                st.image(resized_img/255)
